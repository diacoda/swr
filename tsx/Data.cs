...
public class Data
{
    public static async Task GetData(string ticker, DataFrequency frequency)
    {
        // Define the ticker symbol

        //string tickerSymbol = "CAD-X";// "^GSPC";

        DateTime startDate = new DateTime(1871, 1, 1);
        DateTime endDate = DateTime.Now;

        var yahooFinanceApi = new YahooClient();
        IEnumerable<HistoricalChartInfo> historicalData = await yahooFinanceApi.GetHistoricalDataAsync(ticker, frequency, startDate, endDate);

        // Define the file path
        string filePath = $"{ticker}_daily.csv";
